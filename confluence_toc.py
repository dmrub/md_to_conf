...
def encode_umlauts(s):
    for k,v in UMLAUT_CODES:
        s = s.replace(k, v)
    return s

def decode_umlauts(s):
    for k,v in UMLAUT_CODES:
        s = s.replace(v, k)
    return s
...
def slugify(value, separator):
    ...
    value = encode_umlauts(value)
    value = unicodedata.normalize('NFKD', value).encode('ascii', 'ignore').decode('ascii')
    value = re.sub(r'[^\w\s-]', '', value).strip().lower()
    value = re.sub(r'[%s\s]' % separator, separator, value)
    return decode_umlauts(value)

def confluence_slugify(value, separator=''):
    """ Slugify a string compatible with confluence, to make it URL friendly. """
    value = unicodedata.normalize('NFKD', value).encode('ascii', 'ignore')
    value = re.sub(r'[^\w\s-]', '', value.decode('ascii')).strip()
...
class TocTreeprocessor(Treeprocessor):
    def __init__(self, md, config):
        ...
        self.confluence_title = config["confluence_title"]
    ...
    def run(self, doc):
        ...
        if headings:
            ...
            for heading in headings:
                parent = parent_map[heading]
                heading_index = parent.getchildren().index(heading)
                ...
                anchor = etree.Element("ac:structured-macro", attrib={'ac:name': 'anchor'})
                anchor_param = etree.SubElement(anchor, 'ac:parameter', attrib={'ac:name': ''})
                anchor_param.text = heading.attrib["id"]

                parent.insert(heading_index+1, anchor)
        ...
        # Fix all local references by adding confluence title
        if self.confluence_title:
            href_prefix = confluence_slugify(self.confluence_title)
            for el in doc.iter():
                if isinstance(el.tag, str) and el.tag == 'a':
                    href = el.attrib.get('href')
                    if href and href.startswith('#'):
                        el.attrib['href'] = '#%s-%s' % (href_prefix, href[1:])
...
class ConfluenceTocExtension(Extension):
    ...
    def __init__(self, **kwargs):
        self.config = {
            ...
            "confluence_title": ["",
                                 "Confluence title for link generation"],
            "anchorlink": [False,
...
def makeExtension(**kwargs):  # pragma: no cover
    return ConfluenceTocExtension(**kwargs)
